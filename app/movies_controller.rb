...
def can_get_size_of_the_database
  size = Movie.all.length
  size
end

def can_find_the_first_item_from_the_database_using_id
  first_item = Movie.find(1)
  first_item
end
...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  multi = Movie.find_by(title: "Title", release_date: 2000, director: "Me")
  #binding.pry
  multi
end
...
def can_find_using_where_clause_and_be_sorted
  ...
  sort = Movie.where("release_date > 2002").order(release_date: :desc)
  sort
end

def can_be_found_updated_and_saved
  # Update the title "Awesome Flick" to "Even Awesomer Flick", save it, then return it
  flick = Movie.create(title: "Awesome Flick")
  #Find
